...
export default function QuickActions() {
    ...
    return (
        <div className="bg-gray-800 dark:bg-gray-900 p-3 rounded-lg shadow-md border border-gray-700 dark:border-gray-800">
            <div className="flex flex-wrap gap-2 justify-center items-center">
                {actions.map((action) => (
                    <Link
                        key={action.label}
                        href={action.href}
                        className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-md transition-all text-xs font-medium whitespace-nowrap border border-white/5 hover:border-white/10"
                    >
